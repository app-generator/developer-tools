# [Developer tools](https://en.wikipedia.org/wiki/Web_development_tools)

Usefull tools to automate the development. 

<br />

## [Html Parser](https://en.wikipedia.org/wiki/Beautiful_Soup_(HTML_parser))

A powerfull **Html Parser** coded in Python / Beatiful Soup library capabel to parse pages and extract components for various template engines like Jinja2, CodeIgniter and Laravel. The working flow: 

 - HTML theme is loaded into the parser
 - the user selects a page and have the posibility to walk on HTML tree in a interactive way
 - components can be extracted for Jinja2, Laravel

<br />

## CSS Convertor

This tool, inspired by [Tailwindo](https://github.com/awssat/tailwindo) aims to migrate Bootstrap themes to Bulma and Tailwind. The ideea is to find the corespondence between strauctures and replace the CSS properties. 

<br />

### [Bootstrap](https://getbootstrap.com/) to [Bulma CSS](https://bulma.io/)

@Todo

<br />

### [Bootstrap](https://getbootstrap.com/) to [Tailwind CSS](https://tailwindcss.com/)

@Todo 

<br />

## Support

For issues and features request, use **Github** or access the [support page](https://appseed.us/support) provided by **AppSeed** 

<br />

[Developer tools](https://en.wikipedia.org/wiki/Web_development_tools) provided by **AppSeed**
